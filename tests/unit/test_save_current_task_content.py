        ...
        with patch('builtins.open', mock_file):
            with patch('pathlib.Path.mkdir'):  # Mock directory creation
                with patch.object(manager, 'read_project_info', return_value={"project_id": "test-proj"}):
                    ...
                    handle = mock_file()
                    handle.write.assert_called_once_with(full_data["task"]["description"])
        ...
        with patch('builtins.open', mock_file):
            with patch('pathlib.Path.mkdir'):  # Mock directory creation
                with patch.object(manager, 'read_project_info', return_value={}):
                    with patch.object(manager, 'save_project_info'):
                        manager.save_current_task(full_data, task_group_id="tg-123", task_order=1)
    ...
    def test_save_current_task_real_mcp_data_format(self):
        ...
        real_mcp_data = {
            "task": {
                ...
                "type": "UNDERSTANDING",
                "status": "IN_PROGRESS",
                "task_group_id": "684b158a-a391-490c-9139-010636ebf0ee",
        ...
        with patch('builtins.open', mock_file):
            with patch('pathlib.Path.mkdir'):  # Mock directory creation
                with patch.object(manager, 'read_project_info', return_value={"project_id": "test"}):
                    with patch.object(manager, 'save_project_info'):
                        manager.save_current_task(real_mcp_data, task_group_id="tg-123", task_order=1)
                    
                    # 验证内容正确保存
                    handle = mock_file()
                    expected_content = real_mcp_data["task"]["description"]
