...
class TestSuspendStartWorkflow:
    ...
    async def test_complete_suspend_start_workflow(self, mcp_service, file_manager):
        ...
        with patch('service.get_api_client') as mock_get_client:
            ...
            # Execute suspend
            suspend_result = await mcp_service.suspend_task()
            
            # Verify suspend result
            assert suspend_result["status"] == "success"
            assert "任务组已成功暂存" in suspend_result["message"]
            
            # 验证suspend后in_progress_task为None
            after_suspend_project_info = file_manager.read_project_info()
            assert after_suspend_project_info.get("in_progress_task") is None
            
            # 验证文件被暂存
            suspended_dir = file_manager.suspended_tasks_dir / "task_tg-active"
        ...
        with patch('service.get_api_client') as mock_get_client:
            ...
            # Execute start
            start_result = await mcp_service.start_task("tg-new")
            ...
            assert after_start_project_info["in_progress_task"]["id"] == "tg-new"
        
        # Step 3: 验证完整状态
        final_project_info = file_manager.read_project_info()
        
        # 应该有新的current_task_id
        assert final_project_info["in_progress_task"]["id"] == "tg-new"
        ...
        assert file_manager.current_task_dir.exists()

    @pytest.mark.asyncio
    async def test_cannot_start_when_another_task_active(self, mcp_service, file_manager):
        ...
        with patch('service.get_api_client') as mock_get_client:
            ...
            # 尝试启动新任务组（应该失败）
            result = await mcp_service.start_task("tg-new")
            ...
            assert project_info["in_progress_task"]["id"] == "tg-active"  # 仍然是原来的

    @pytest.mark.asyncio
    async def test_start_when_no_active_task(self, mcp_service, file_manager):
        ...
        with patch('service.get_api_client') as mock_get_client:
            ...
            # Execute start
            result = await mcp_service.start_task("tg-first")
            ...
            assert updated_project_info["in_progress_task"]["id"] == "tg-first"